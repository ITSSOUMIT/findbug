...
module Findbug
  module Jobs
    ...
    #
    #    every 30.seconds do
    #      runner "Findbug::Jobs::PersistJob.perform_now"
    #    end
    #
    # 3. Solid Queue (Rails 8):
    #
    #    Findbug::Jobs::PersistJob.set(wait: 30.seconds).perform_later
    ...
    class PersistJob < ActiveJob::Base
      ...
      def persist_errors
        ...
        loop do
          ...
          events = Storage::RedisBuffer.pop_errors(batch_size)
      ...
      def persist_performance
        ...
        loop do
          events = Storage::RedisBuffer.pop_performance(batch_size)
          break if events.empty?
      ...
      class << self
        ...
        def persist_errors_batch(events)
          events.each do |event_data|
            ...
            scrubbed = Processing::DataScrubber.scrub(event_data)

            # Upsert to database
            ErrorEvent.upsert_from_event(scrubbed)
          rescue StandardError => e
